...
use crate::{errors::FundError, state::{Investment, InvestmentFund}};
...
pub struct RegisterInvestment<'info> {
    ...
    #[account(
        init,
        seeds = [b"investment", identifier.as_bytes(), investment_fund.key().as_ref()],
        bump,
        payer = manager,
        space = Investment::get_size(&identifier)
    )]
